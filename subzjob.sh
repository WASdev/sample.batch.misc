...
$LOCATION $ACTION $SERVER $APP $JOBXML $PARM1 $PARM2 $PARM3 $PARM4 > $OUTLOC/out.txt 2> $OUTLOC/err.txt
...
if [ rc -eq 35 ]; then
  ...
  exitcode=4
  echo 'batchManagerZos exit code=' $rc 'Status=Issue: someone stopped the job.'
  echo 'Will set shell script exit code to' $exitcode
  ...
  exitcode=8
  echo 'batchManagerZos exit code=' $rc 'Status=Issue: argument problem -- required, unrecognized, invalid.'
  echo 'Will set shell script exit code to' $exitcode
  ...
  exitcode=12
  echo 'batchManagerZos exit code=' $rc 'Status=Problem: did not complete or was abandoned.'
  echo 'Will set shell script exit code to' $exitcode
  ...
  exitcode=13
  echo 'batchManagerZos exit code=' $rc 'Status=Problem: some other problem occurred.  See Knowledge Center.'
  echo 'Will set shell script exit code to' $exitcode
